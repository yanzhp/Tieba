...
def readDocument():
    ...
    db = pymysql.connect(host='localhost', user='root', password='yjs43520100', port=3306, db="spiders",
                         charset='utf8', )
    sql = "SELECT context FROM tiebaconlist"
...
def segment(doc):
    ...
    document_after_segment = open('分词结果_内容.txt', 'w+',encoding='utf-8')
...
def wordCount(segment_list):
    ...
    word_lst = []
    word_dict = {}
    with open('词频统计(去停用词)_内容.txt','w',encoding='utf-8') as wf2:
...
def drawWordCloud(seg_list):
    ...
    wc.to_file("savegif/contentciyun.jpg") #保存图片
    #  显示词云图片
...
def removeStopWords(seg_list):
    ...
    stop_words = open('CNstopwords.txt',encoding='utf-8')
    ...
    without_stopwords = open('分词结果(去停用词)_内容.txt', 'w',encoding='utf-8')
...
def drawStatBarh():
    ...
    words = []
    counts = []
    for line in open('词频统计(去停用词)_内容.txt',encoding='utf-8'):
        line.strip('\n')
        if len(line.split()) > 1:
    ...
    ax.set_title('贴吧回复内容高频词',fontproperties=myfont, fontsize=12)
    ax.set_xlabel(u"出现次数",fontproperties=myfont)
    plt.savefig("savegif/贴吧回复内容高频词统计图.png")
    autolabel(rects, ax)
    plt.show()
