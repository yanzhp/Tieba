...
def segment(doc):
    ...
    document_after_segment = open('分词结果_标题.txt', 'w+',encoding='utf-8')
...
def wordCount(segment_list):
    ...
    word_lst = []
    word_dict = {}
    with open('词频统计(去停用词)_标题.txt','w',encoding='utf-8') as wf2:
...
def removeStopWords(seg_list):
    ...
    without_stopwords = open('分词结果(去停用词)_标题.txt', 'w',encoding='utf-8')
...
def drawStatBarh():
    ...
    words = []
    counts = []
    for line in open('词频统计(去停用词)_标题.txt',encoding='utf-8'):
        line.strip('\n')
        if len(line.split()) > 1:
    ...
    plt.savefig("savegif/贴吧标题高频词统计图.png")
    autolabel(rects, ax)
    plt.show()
